...
Office.onReady((info) => {
    ...
    // Add debugging for desktop vs web
    if (info.platform === Office.PlatformType.PC) {
        console.log('Running on PowerPoint Desktop');
    } else if (info.platform === Office.PlatformType.OfficeOnline) {
        console.log('Running on PowerPoint Web');
    } else {
        console.log('Running on unknown platform:', info.platform);
    }
...
async function testApiConnectivity() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (resultDiv) {
                resultDiv.innerHTML = `<p style="color: green;">✅ API Connection Successful!<br>Status: ${data.status}<br>API Base: ${apiBase}</p>`;
            }
            return true;
        } else {
            console.error('Health check failed:', response.status);
            if (resultDiv) {
                resultDiv.innerHTML = `<p style="color: red;">❌ API Connection Failed!<br>Status: ${response.status} ${response.statusText}<br>API Base: ${apiBase}</p>`;
            }
            return false;
        }
    } catch (error) {
        ...
        if (resultDiv) {
            resultDiv.innerHTML = `<p style="color: red;">❌ API Connection Error!<br>Error: ${error.message}<br>API Base: ${apiBase}</p>`;
        }
        return false;
    }
}

// Test login process specifically
async function testLoginProcess() {
    console.log('Testing login process...');
    const apiBase = await getApiBase();
    console.log('API Base for login:', apiBase);
    
    const resultDiv = document.getElementById('api-result');
    if (resultDiv) {
        resultDiv.innerHTML = '<p>Testing login process...</p>';
    }
    ...
    try {
        const response = await fetch(`${apiBase}/auth/login/`, {
            ...
            body: JSON.stringify({ username: 'instructor1', password: 'password' })
        });
        ...
        if (response.ok) {
            ...
            if (resultDiv) {
                resultDiv.innerHTML = `<p style="color: green;">✅ Login Test Successful!<br>Token received: ${data.access ? 'Yes' : 'No'}<br>API Base: ${apiBase}</p>`;
            }
            return true;
        } else {
            ...
            if (resultDiv) {
                resultDiv.innerHTML = `<p style="color: red;">❌ Login Test Failed!<br>Status: ${response.status}<br>Error: ${errorData.detail || 'Unknown error'}<br>API Base: ${apiBase}</p>`;
            }
            return false;
        }
    } catch (error) {
        ...
        if (resultDiv) {
            resultDiv.innerHTML = `<p style="color: red;">❌ Login Test Error!<br>Error: ${error.message}<br>API Base: ${apiBase}</p>`;
        }
        return false;
    }
}
...
// Upload and insert image to PowerPoint slide
async function uploadAndInsert() {
    ...
    formData.append("image", file); // Use 'image' field as expected by Django
    formData.append("question_text", "Image uploaded from PowerPoint add-in"); // Required field
    formData.append("class_id", "1"); // Use a default class ID for now

    // Django API endpoint for questions
    const apiBase = await getApiBase();
    const apiUrl = `${apiBase}/questions/`;
    ...
    try {
        ...
        const data = await response.json();
        const imageUrl = data.image_url; // Use 'image_url' as returned by Django
...
function initializeEventListeners() {
    ...
    // Upload button for PowerPoint insertion
    ...
    if (testConnectivityButton) {
        testConnectivityButton.addEventListener('click', testApiConnectivity);
    }
    
    // Test Login button
    const testLoginButton = document.getElementById('test-login');
    if (testLoginButton) {
        testLoginButton.addEventListener('click', testLoginProcess);
    }
...
async function handleLogin(event) {
    ...
    try {
        ...
        // Enhanced error handling for desktop vs web
        let errorMessage = error.message;
        
        if (error.name === 'TypeError' && error.message.includes('fetch')) {
            errorMessage = 'Network error: Unable to connect to backend server. Please check if the server is running.';
        } else if (error.message.includes('CORS')) {
            errorMessage = 'CORS error: Cross-origin request blocked. This may be a desktop security issue.';
        } else if (error.message.includes('Load failed')) {
            errorMessage = 'Load failed: This is a common issue in PowerPoint Desktop. Try refreshing the add-in or check the console for more details.';
        }
